...
mcreateChart = function (name) {
   ...
   //          	}
	...
	var mupdateChannelShow = function(data){
		channel = $.extend(channel, data.channel);
		setFields(data);
		minitData(data);
	}

	var mupdateChannelMultiShow = function(data){
		channel = $.extend(channel, data.channel);
		setFields(data);
		console.log(data);
		var template = $('<iframe src="mshow.html" width="100%" height="400px" style="border: 0px"></iframe>');
		...
		for (fieldi in channel.data){
			...
			var src = "mshow.html?"+$.param( $.extend(params_clone, {fields:field.field, types:field.type} ) );
			$iframe.attr("src", src);
			$('#mcontainer').append($iframe);
		}
	}

	function mgetChannelInfo(callback){
	...
	function minitData(object){
		...
		$.each(channel.list, function (i, name) {
			...
			$.getJSON(fetch_url,    function (data) {
				...
				seriesCounter += 1;
				console.log(channel.data[name].name);
				if (seriesCounter === channel.names.length) {
					mcreateChart(channel.data[name].name);
